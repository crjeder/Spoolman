use spoolman_types::{requests::{CreateSpool, UpdateSpool}, responses::SpoolResponse};
...
pub async fn list_spools(allow_archived: bool) -> Result<Vec<SpoolResponse>, ApiError> {
    get(&format!("/api/v1/spool?allow_archived={allow_archived}&order=desc")).await
}
...
pub async fn clone_spool(id: u32) -> Result<SpoolResponse, ApiError> {
    post_json(&format!("/api/v1/spool/{id}/clone"), &serde_json::Value::Null).await
}
